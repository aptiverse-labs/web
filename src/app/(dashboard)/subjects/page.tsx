...
import { BookOpen, ChevronRight, Edit, MoreVertical, Plus, Search, Trash2 } from 'lucide-react'
...
const Subjects = () => {
  ...
  const { student } = useStudentData(4);

  const filteredSubjects = student?.studentSubjects.filter(subject =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-4 px-4 sm:px-6 lg:px-8">
            ...
            <div key={subject.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3 flex-1">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${subject.subjectColor}`}>
                    <BookOpen size={16} className="text-white" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-gray-900 truncate">{subject.subjectName}</h3>
                    <p className="text-xs text-gray-500 font-mono mt-0.5">{subject.subjectCode}</p>
                  </div>
                </div>
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-semibold text-gray-900">{subject.progress || 0}%</span>
                </div>
                <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-300 ${subject.subjectColor}`}
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="text-center p-2 bg-gray-50 rounded-lg">
                  <div className={`font-semibold ${subject?.upcomingDeadlines ? 'text-yellow-700' : 'text-gray-600'
                    }`}>
                  ...
                  <tr key={subject.id} className="hover:bg-gray-50 transition-colors duration-150">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${subject.subjectColor}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-600 font-mono">{subject.subjectCode}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3 min-w-[120px]">
                        <span className="text-sm font-medium text-gray-900 w-8">{subject.progress || 0}%</span>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-medium text-gray-900">{subject.averageScore || 0}%</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-600">{subject.studyHours || 0}h</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-600">{subject.assignmentsCompleted || 0}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${subject.upcomingDeadlines
                        ? 'bg-yellow-100 text-yellow-800'
                        : 'bg-gray-100 text-gray-600'
                        }`}>
                        {subject.upcomingDeadlines || "Empty"}
                      </span>
                    </td>
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-70 flex items-end sm:items-center justify-center z-50 p-4 sm:p-6">
