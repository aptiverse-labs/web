...
// Rand to the cent. The shared formatCurrency drops the decimals, which is
// fine for an MRR headline and wrong here: R25.80 a month is the number, and
// showing it as R26 would misstate what somebody is owed.
...
export default function ReferPage() {
  ...
  return (
    <>
      ...
      {summary.isLoading && <Skeleton variant="rounded" height={220} sx={{ mb: 3 }} />}

      {summary.isError && (
        <Alert severity="error" sx={{ mb: 3 }}>
          We could not load your referral page just now. Please refresh.
        </Alert>
      )}
      ...
      {summary.data && (
        <Stack spacing={3}>
          ...
          {summary.data.needsPayoutDetails && summary.data.payoutOnboardingStatus === "not_started" && (
            <Alert severity="info" icon={<Wallet size={18} />}>
              You have money waiting. Add your ID and bank details on the
              <strong> Get paid </strong> tab so it can be paid out.
            </Alert>
          )}
          {summary.data.payoutOnboardingStatus === "submitted" && (
          ...
          {summary.data.payoutOnboardingStatus === "rejected" && (
            <Alert severity="warning">
              We could not verify your payout details.
              {summary.data.reviewNotes ? ` ${summary.data.reviewNotes}` : ""} Please correct them on
              the <strong>Get paid</strong> tab. Nothing you have earned is lost.
            </Alert>
          )}
          ...
          <Card>
            ...
            >
              <Tab label={`People (${summary.data.referralsTotal})`} />
              <Tab label="Earnings" />
              <Tab label="Payments to you" />
              <Tab label="Get paid" />
              <Tab label="Tax statement" />
            </Tabs>
            <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
              {tab === 0 && <ReferralsTab />}
              {tab === 1 && <CommissionsTab />}
              {tab === 2 && <PayoutsTab />}
              {tab === 3 && <PayoutDetailsForm summary={summary.data} />}
              {tab === 4 && <TaxStatementTab />}
            </CardContent>
          </Card>
...
// first thing on the page at display size, and the four states the money can
// be in sit directly under it rather than behind a tab.
function EarningsHero({ data }: { data: AffiliateSummary }) {
          ...
          <Grid size={{ xs: 12, md: 7 }}>
            <Box
              sx={(t) => ({
                p: 2,
                borderRadius: 3,
                // Citron as a surface with graphite ink on it, never as
                // coloured type.
...
function MoneyTile({ label, value, hint }: { label: string; value: number; hint: string }) {
  return (
    <Grid size={{ xs: 6, md: 3 }}>
      <Typography variant="overline" color="text.secondary">
        {label}
        ...
        >
          ...
          <Box sx={{ textAlign: "right", flexShrink: 0 }}>
            <Typography
              sx={{
                fontWeight: 700,
                color: e.amountZar < 0 ? "error.main" : "text.primary",
              }}
            >
              {rand(e.amountZar)}
            </Typography>
...
function TaxStatementTab() {
  ...
  const selected = useMemo(
    () => years.find((y) => y.taxYear === year) ?? years[0],
    [years, year],
  );
...
function HowItWorks({ data }: { data: AffiliateSummary }) {
  const items = [
    {
