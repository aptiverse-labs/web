...
export default function ParentLivePage() {
  ...
  const [series, setSeries] = useState<number[][]>(CHILDREN.map(() => Array.from({ length: 30 }, () => Math.round(Math.random() * 100))));
              ...
              <Card key={c.id}>
                <CardContent sx={{ p: 3 }}>
                  ...
                  <LineChart
                    ...
                    yAxis={[{ min: 0, max: 100 }]}
                    series={[{ data: series[i], curve: "natural", color: "#1F8079", showMark: false, area: true }]}
